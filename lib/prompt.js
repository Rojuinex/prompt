...
prompt.start = function (options) {
  ...
  prompt.rootMessage = options.rootMessage || prompt.rootMessage; 
...
prompt.getInput = function (prop, callback) {
  ...
  //
  // If there is a stored schema for `propName` in `prompt.properties`
  // then use it.
  //
  ...
  if(prompt.rootMessage)
    raw.push(prompt.message);

  if(prompt.colors) {
    raw.push(prompt.rootMessage? delim + name.grey : name.grey);
  } else {
    raw.push(prompt.rootMessage? delim + name : name);
  }

  raw.push(prompt.colors? delim.grey : delim);
